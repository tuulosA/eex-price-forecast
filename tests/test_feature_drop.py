...
from eex_forecast.feature_drop import (
    feature_names,
    resolve_selection,
    run_feature_drop,
    save_drop_report,
...
def test_run_feature_drop_guards() -> None:
    frame = make_timeseries(periods=24 * 120)
    spec = REGISTRY["wind"]
    names = feature_names(spec, frame)
    with pytest.raises(ValueError, match="No features selected"):
        run_feature_drop(spec, frame, [], params=TINY, n_cutoffs=2, horizon_hours=48)
...
def test_save_drop_report(tmp_path: Path) -> None:
    frame = make_timeseries(periods=24 * 120)
    spec = REGISTRY["price"]
    result = run_feature_drop(
        spec,
    ...
    path = save_drop_report(result, reports_dir=tmp_path)
    assert path.name == "price_drop.json"
