"""Tests for the fundamentals weather-aggregation A/B tool."""
...
def test_run_aggregation_ranks_all_wind_strategies() -> None:
...
def test_run_aggregation_solar_and_load() -> None:
    frame = make_timeseries(periods=24 * 120)
...
def test_run_aggregation_rejects_unknown_strategy() -> None:
    frame = make_timeseries(periods=24 * 120)
...
def test_save_aggregation_report_writes_ranking(tmp_path: Path) -> None:
    ...
    path = save_aggregation_report(result, reports_dir=tmp_path)
    assert path.name == "wind_aggregation.json"
...
def test_save_neighbour_aggregation_report(tmp_path: Path) -> None:
    ...
    payload = json.loads(path.read_text())
    assert path.name == "neighbour_aggregation.json"
    assert payload["model"] == "price" and payload["compared"] == "neighbour-wind aggregation"
